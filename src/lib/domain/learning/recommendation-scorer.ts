import { LearningResource, LearnerPreferences, TechnologyEcosystem } from "../../contracts";
import { isResourceCompatible } from "./technology-ecosystem";

export class RecommendationScorer {
  /**
   * Ranks curated learning resources using explainable multi-factor scoring with deterministic ecosystem compatibility.
  ...
  public scoreAndRankResources(
    ...
    const scored = resources
      .filter((res) => {
        if (targetEcosystem !== "agnostic" && !isResourceCompatible(res.ecosystem, targetEcosystem)) {
          return false;
        }
        return true;
      })
      .map((res) => {
        // Relevance (0 to 1)
        let relevance = res.skillId === skillId ? 1.0 : 0.4;

        if (res.languageOrDomainMatch && targetEcosystem !== "agnostic") {
