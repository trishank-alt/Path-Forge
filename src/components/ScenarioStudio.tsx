...
export function ScenarioStudio({
  ...
  return (
    <div className="space-y-6">
      ...
      {latestScenario && (latestScenario.computedRoadmap || latestScenario.resultRoadmap) && latestScenario.diff && (() => {
        const activeRoadmap = (latestScenario.computedRoadmap || latestScenario.resultRoadmap)!;
        const diff = latestScenario.diff!;
        return (
          <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-950/40 to-slate-900/60 border border-indigo-500/30 glow-indigo space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-300 mt-0.5">{diff.summary}</p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                ...
                <div className="flex items-baseline gap-1.5">
                  <span className="text-base font-bold font-mono text-slate-100">
                    {activeRoadmap.totalEstimatedWeeks} wks
                  </span>
                  <span
              ...
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                ...
                <div className="flex items-baseline gap-1.5">
                  <span className="text-base font-bold font-mono text-slate-100">
                    {activeRoadmap.totalEstimatedHours} hrs
                  </span>
                  <span className="text-xs font-mono text-slate-400">
                    ({activeRoadmap.weeklyPaceHours}h/wk)
                  </span>
                </div>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                ...
                <span className="text-base font-bold font-mono text-emerald-400 flex items-center gap-1">
                  ...
                  {diff.addedSkillIds.length} skills
                </span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                ...
                <span className="text-base font-bold font-mono text-rose-400 flex items-center gap-1">
                  ...
                  {diff.removedSkillIds.length} skills
                </span>
              </div>
            </div>
          </div>
